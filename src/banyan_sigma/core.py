...
solver_output_columns = ['LN_P','D_OPT','RV_OPT','ED_OPT','ERV_OPT','X','Y','Z','U','V','W','EX','EY','EZ','EU','EV','EW','XYZ_SEP','UVW_SEP','XYZ_SIG','UVW_SIG','MAHALANOBIS']
...
def membership_probability(stars_data=None,column_names=None,hypotheses=None,ln_priors=None,ntargets_max=1e7,ra=None,dec=None,pmra=None,pmdec=None,epmra=None,epmdec=None,dist=None,edist=None,rv=None,erv=None,psira=None,psidec=None,epsira=None,epsidec=None,plx=None,eplx=None,constraint_dist_per_hyp=None,constraint_edist_per_hyp=None,restrained_distance_range=None,unit_priors=False,lnp_only=False,no_xyz=False,use_rv=None,use_dist=None,use_plx=None,use_psi=None,custom_models=None):
	...
	#Format the distance ranges used to skip model evaluations
	if restrained_distance_range is not None:
		if 'DISTANCE_MIN' not in parameters_str.colnames or 'DISTANCE_MAX' not in parameters_str.colnames:
			raise ValueError('If restrained_distance_range is used, the models must include DISTANCE_MIN and DISTANCE_MAX columns')
		restrained_distance_range = np.asarray(restrained_distance_range,dtype=float)
		if restrained_distance_range.shape == (2,):
			restrained_distance_range = np.tile(restrained_distance_range,(nobj,1))
		elif restrained_distance_range.shape == (2,nobj):
			restrained_distance_range = restrained_distance_range.transpose()
		elif restrained_distance_range.shape != (nobj,2):
			raise ValueError('restrained_distance_range must be a 2-elements range, a 2xN array, or an Nx2 array')
		if np.max(~np.isfinite(restrained_distance_range)):
			raise ValueError('restrained_distance_range must contain finite values')
		if np.max((restrained_distance_range[:,0] < 0.) | (restrained_distance_range[:,1] < 0.) | (restrained_distance_range[:,0] > restrained_distance_range[:,1])):
			raise ValueError('restrained_distance_range values must be physical and ordered as [minimum, maximum]')
	...
	hypotheses = np.array([hyp.upper().decode('UTF-8') if isinstance(hyp,bytes) else str(hyp).upper() for hyp in np.asarray(hypotheses).tolist()])
	...
	for i in range(nhyp):
		...
		if lnp_only is True:
			all_lnprobs_hypi = np.full((nobj,ngauss),-np.inf)
		...
		for gaussi in range(ngauss):
			...
			selected_stars = None
			if restrained_distance_range is not None and hypotheses[i] != 'FIELD':
				selected_stars = (parameters_str_row['DISTANCE_MIN'] <= restrained_distance_range[:,1]) & (parameters_str_row['DISTANCE_MAX'] >= restrained_distance_range[:,0])
			...
			for ci in range(nbatches):
				...
				nobj_ci = data_table_ci.shape[0]
				selected_stars_ci = None
				if selected_stars is not None:
					selected_stars_ci = selected_stars[ind_from:ind_to]

				#Solve the BANYAN Sigma integrals for this hypothesis and this batch of targets
				if selected_stars_ci is None:
					...
					output_str_ci = empty_solver_output(nobj_ci)
					if np.any(selected_stars_ci):
						data_table_ci_selected = data_table_ci.iloc[selected_stars_ci]
						output_str_ci_selected = banyan_sigma_solve_multivar(data_table_ci_selected['RA'].values,data_table_ci_selected['DEC'].values,data_table_ci_selected['PMRA'].values,data_table_ci_selected['PMDEC'].values,data_table_ci_selected['EPMRA'].values,data_table_ci_selected['EPMDEC'].values,rv_measured=data_table_ci_selected['RV'].values,rv_error=data_table_ci_selected['ERV'].values,dist_measured=dist_for_this_hypothesis_ci[selected_stars_ci],dist_error=edist_for_this_hypothesis_ci[selected_stars_ci],psira=data_table_ci_selected['PSIRA'].values,psidec=data_table_ci_selected['PSIDEC'].values,psira_error=data_table_ci_selected['EPSIRA'].values,psidec_error=data_table_ci_selected['EPSIDEC'].values,precision_matrix=parameters_str_row['PRECISION_MATRIX'],center_vec=parameters_str_row['CENTER_VEC'],precision_matrix_determinant=parameters_str_row['PRECISION_DETERM'])
						output_str_ci.iloc[np.where(selected_stars_ci)[0],:] = output_str_ci_selected[solver_output_columns].values

				#Store the log of probabilities if those are the only required output
				if lnp_only is True:
					all_lnprobs_hypi[ind_from:ind_to,gaussi] = output_str_ci['LN_P'].values
					continue
		...
		if lnp_only is True:
			if ngauss == 1:
				all_lnprobs[:,i] = all_lnprobs_hypi[:,0]
			else:
	...
	lnprobs_ymg = all_lnprobs[:,yind]
	ln_norm_ymg = logsumexp(lnprobs_ymg,axis=1)
	ln_norm_output_only_ymg = np.full(lnprobs_ymg.shape,-np.inf)
	good_norm_ymg = np.isfinite(ln_norm_ymg)
	ln_norm_output_only_ymg[good_norm_ymg,:] = lnprobs_ymg[good_norm_ymg,:] - ln_norm_ymg[good_norm_ymg,None]
	...
	ln_prior_moving_groups[~np.isfinite(ln_prior_moving_groups)] = 0.
...
def banyan_sigma_solve_multivar(ra,dec,pmra,pmdec,pmra_error,pmdec_error,precision_matrix=None,center_vec=None,rv_measured=None,rv_error=None,dist_measured=None,dist_error=None,psira=None,psidec=None,psira_error=None,psidec_error=None,lnP_only=False,precision_matrix_determinant=None,debug=False):
	...
	output_structure = pd.DataFrame(np.array([lnP,dist_optimal,rv_optimal,edist_optimal,erv_optimal,X,Y,Z,U,V,W,EX,EY,EZ,EU,EV,EW,XYZ_sep,UVW_sep,XYZ_sig,UVW_sig,mahalanobis]).transpose(),columns=solver_output_columns)

	#Return the output table
	return output_structure

def empty_solver_output(nobj):
	output_structure = pd.DataFrame(np.full((nobj,len(solver_output_columns)),np.nan),columns=solver_output_columns)
	output_structure['LN_P'] = -np.inf
	return output_structure
